...
#    3. Health:     /api/health → version V7.3.0 + build_hash == local git SHA
#    4. UI version: / contains <meta name="voy-version" content="V7.3.0">
...
if [ -n "$HEALTH" ]; then
  ...
  printf "  version:    %s\n" "${LIVE_VER:-<missing>}"
  printf "  build_hash: %s\n" "${LIVE_HASH:-<missing>}"
  if [ "$LIVE_VER" = "V7.3.0" ]; then
    ok "Worker version is V7.3.0"
  else
    bad "Worker version is '$LIVE_VER' (expected V7.3.0) — STALE WORKER deployed"
  fi
...
if echo "$HTML" | grep -q 'voy-version" content="V7.3.0"'; then
  ok "UI HTML contains V7.3.0 version pin"
else
  if echo "$HTML" | grep -q 'voy-version'; then
    LIVE_UI_VER=$(echo "$HTML" | grep -o 'voy-version" content="[^"]*"' | cut -d'"' -f4 || echo "")
    bad "UI version is '$LIVE_UI_VER' (expected V7.3.0) — STALE HTML on edge"
  else
...
if [ "$ENTRY_CODE" = "301" ] || [ "$ENTRY_CODE" = "308" ]; then
  ...
  if echo "$ENTRY_HTML" | grep -q 'voy-version" content="V7.3.0"'; then
    ok "/VOY-Lite.html → 200 (CF Assets direct serve, V7.3.0 content verified — path not hidden but app correct)"
  else
    bad "/VOY-Lite.html → 200 but content is NOT V7.3.0 (stale V4 serving from this path)"
  fi
else
