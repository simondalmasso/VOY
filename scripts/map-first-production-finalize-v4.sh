...
EVIDENCE_DIR="${EVIDENCE_DIR:-test-results/map-first-production-finalize-v4}"
...
export EVIDENCE_DIR STABLE_VERSION_ID CANDIDATE_VERSION_ID EXPECTED_STABLE_VERSION EXPECTED_STABLE_HASH EXPECTED_CANDIDATE_VERSION CANDIDATE_SHORT_SHA CANDIDATE_SOURCE_SHA
...
verify_stable_round() {
  local suffix="$1"
  [[ "$(control_state "$suffix")" == stable && "$(health_state "$suffix")" == stable ]]
}
...
rollback_once() {
  ...
  wrangler versions deploy "$STABLE_VERSION_ID@100%" "$CANDIDATE_VERSION_ID@0%" --message "VOY Map-First single rollback after failed production validation" --yes | tee -a "$EVIDENCE_DIR/rollback.log"
  local consecutive=0
  for round in $(seq 1 20); do
    fetch_snapshot "rollback-${round}"
    if verify_stable_round "rollback-${round}"; then
...
# The rollback guard is armed before the first operation that can leave candidate traffic active.
trap post_candidate_failure ERR

fetch_snapshot entry
initial_control="$(control_state entry)"
initial_health="$(health_state entry)"
printf 'CONTROL=%s\nHEALTH=%s\n' "$initial_control" "$initial_health" | tee "$EVIDENCE_DIR/entry-state.txt"

case "$initial_control" in
  candidate)
    candidate_active=1
    echo 'ENTRY_ALREADY_PROMOTED=YES' | tee "$EVIDENCE_DIR/promotion-state.txt"
    ;;
  stable)
    if [[ "$initial_health" != stable ]]; then
      echo "stable_control_with_unexpected_health=$initial_health" >&2
      false
    fi
    ...
    ;;
  *)
    echo "unexpected_control_state=$initial_control" >&2
    # Any ambiguous traffic state is restored to the known stable split once, then stopped.
    candidate_active=1
    rollback_once
    candidate_active=0
    exit 21
...
# Edge health may legitimately lag the control-plane change. Do not fail on a stale first response.
start_ms=$(date +%s%3N)
consecutive=0
final_deployment_id=''
...
for round in $(seq 1 60); do
  ...
  if [[ "$cs" == candidate && "$hs" == candidate ]]; then
    consecutive=$((consecutive+1))
    ok=true
  else
    consecutive=0
  ...
  final_deployment_id="$(node -e "try{const p=require('./$EVIDENCE_DIR/deployments-converge-$round.json');const a=(p.result?.deployments||p.result||[])[0];process.stdout.write(a?.id||'')}catch{}")"
  printf '{"round":%d,"elapsed_ms":%d,"control":"%s","health":"%s","ok":%s,"consecutive":%d,"deployment_id":"%s"}\n' "$round" "$elapsed" "$cs" "$hs" "$ok" "$consecutive" "$final_deployment_id" >> "$EVIDENCE_DIR/production-convergence.jsonl"
...
# Observe the exact production version while the full public-browser suite runs.
