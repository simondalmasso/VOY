...
EXPECTED_STABLE_HASH="${EXPECTED_STABLE_HASH:-1f1e1d3}"
EXPECTED_STABLE_VERSION_ID="${EXPECTED_STABLE_VERSION_ID:-2ee28f09-1aa8-4250-bb2c-ba4684a07791}"
EXPECTED_CANDIDATE_VERSION="${EXPECTED_CANDIDATE_VERSION:-V8.0.0}"
EVIDENCE_DIR="${EVIDENCE_DIR:-test-results/hotfix-map-mobile-candidate}"
...
CANDIDATE_CONFIG="$(dirname "$DEPLOY_CONFIG")/hotfix-candidate-config.json"
...
node --check scripts/hotfix-candidate-final.mjs
bash -n scripts/run-browser-smoke.sh scripts/run-hotfix-candidate.sh
...
curl --fail --silent --show-error --retry 3 -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" "$API_ROOT/versions?per_page=100" > /tmp/hotfix-versions-before.json
curl --fail --silent --show-error --retry 3 "$WORKER_URL/api/health?hotfix_preflight=$(date +%s%N)" > "$EVIDENCE_DIR/production-health-before.json"
...
curl --fail --silent --show-error --retry 3 -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" "$API_ROOT/versions/$STABLE_VERSION_ID" > /tmp/hotfix-stable-version.json

VOY_BASE_URL="$WORKER_URL" VOY_EXTERNAL_SERVER=1 VOY_CAPTURE_HOTFIX_BASELINE=1 VOY_EVIDENCE_DIR="$EVIDENCE_DIR/production-before-screens" VOY_OUTPUT_DIR="$EVIDENCE_DIR/production-before-output" VOY_REPORT_DIR="$EVIDENCE_DIR/production-before-report" bunx playwright test -c browser-tests/playwright.config.ts --project=mobile-360x800 -g 'hotfix baseline captures current production before candidate' 2>&1 | tee "$EVIDENCE_DIR/production-before-browser.log"

CANDIDATE_TAG="voy-hotfix-map-mobile-${GITHUB_SHA:0:12}"
export CANDIDATE_TAG
wrangler versions upload --config "$CANDIDATE_CONFIG" --minify --tag "$CANDIDATE_TAG" --message "VOY post-release map/mobile UI hotfix candidate $GITHUB_SHA" 2>&1 | tee "$EVIDENCE_DIR/version-upload.log"
curl --fail --silent --show-error --retry 3 -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" "$API_ROOT/versions?per_page=100" > /tmp/hotfix-versions-after.json
...
const before=new Set(versions('/tmp/hotfix-versions-before.json').map(v=>v.id)); const added=versions('/tmp/hotfix-versions-after.json').filter(v=>v.id&&!before.has(v.id)); if(added.length!==1)throw new Error(`candidate_version_not_unique:${added.length}`);
...
curl --fail --silent --show-error --retry 3 -H "Authorization: Bearer $CLOUDFLARE_API_TOKEN" "$API_ROOT/versions/$CANDIDATE_VERSION_ID" > /tmp/hotfix-candidate-version.json
node <<'NODE'
const fs=require('node:fs');
...
const stable=bindings('/tmp/hotfix-stable-version.json'), candidate=bindings('/tmp/hotfix-candidate-version.json');
const left=stable.map(x=>`${x.name}:${x.type}`), right=candidate.map(x=>`${x.name}:${x.type}`); if(JSON.stringify(left)!==JSON.stringify(right))throw new Error(`binding_contract_drift:${JSON.stringify({stable:left,candidate:right})}`);
fs.writeFileSync(`${process.env.EVIDENCE_DIR}/binding-contract.json`,JSON.stringify({result:'PASS',stable_binding_count:stable.length,candidate_binding_count:candidate.length,stable_bindings:stable,candidate_bindings:candidate},null,2));
NODE

wrangler versions deploy "$STABLE_VERSION_ID@100%" "$CANDIDATE_VERSION_ID@0%" --message "VOY hotfix candidate 0% $GITHUB_SHA" --yes 2>&1 | tee "$EVIDENCE_DIR/candidate-deployment.log"
TAIL_PID=''
stop_tail(){ if [[ -n "$TAIL_PID" ]] && kill -0 "$TAIL_PID" 2>/dev/null; then kill "$TAIL_PID" 2>/dev/null||true; wait "$TAIL_PID" 2>/dev/null||true; fi; }
trap stop_tail EXIT
timeout 1200s wrangler tail "$WORKER_NAME" --format json --version-id "$CANDIDATE_VERSION_ID" > "$EVIDENCE_DIR/candidate-tail.log" 2>&1 & TAIL_PID=$!
sleep 5
...
stop_tail; trap - EXIT
node scripts/hotfix-candidate-final.mjs | tee "$EVIDENCE_DIR/final-state.log"

curl --fail --silent --show-error --retry 3 "$WORKER_URL/api/health?hotfix_after=$(date +%s%N)" > "$EVIDENCE_DIR/production-health-after.json"
node <<'NODE'
