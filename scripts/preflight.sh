...
if grep -q 'WORKER_VERSION = "V7.5.0"' worker.js; then ok "WORKER_VERSION = V7.5.0"; else bad "WORKER_VERSION missing"; fi
...
if grep -q 'voy-version" content="V7.5.0"' public/VOY-Lite.html; then ok "meta voy-version = V7.5.0"; else bad "version meta missing"; fi
if grep -q "window.VOY_VERSION='V7.5.0'" public/VOY-Lite.html; then ok "window.VOY_VERSION = V7.5.0"; else bad "VOY_VERSION JS missing"; fi
