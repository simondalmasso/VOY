...
if (playwrightPackage.version !== '1.61.1') throw new Error(`unexpected_playwright_version:${playwrightPackage.version}`);
...
fs.writeFileSync(path.join(out, 'playwright-resolution.json'), `${JSON.stringify({ result: 'PASS', version: playwrightPackage.version, resolved_entrypoint: resolvedPlaywright, script_path: import.meta.url, script_inside_repository: import.meta.url.includes('/scripts/issue36-before-capture.mjs') }, null, 2)}\n`);

const terminal = {
  canonicalId: 'santafe:landmark:terminal-omnibus', nombre: 'Terminal de Ómnibus', aliases: ['terminal'], verified: true,
  source: 'authoritative', precision: 'poi', lat: -31.643533, lon: -60.700503, address: 'Belgrano 2910', verified_at: '2026-08-05',
...
const transportFixture = { schema_version: 2, city_id: 'santafe', verified_at: '2026-08-05', landmarks: [terminal], bus_routes: [], bus_stops: [], bike_stations: [] };
...
async function setup(page, { destinationMode = 'normal' } = {}) {
  await page.route('**/cities/santa-fe/transport.json', route => route.fulfill({ status: 200, contentType: 'application/json', body: JSON.stringify(transportFixture) }));
...
async function plan(page) {
  await setManualOrigin(page); await selectDestination(page);
...
for (const [label,width,height] of viewports) {
  for (const scheme of ['light','dark']) {
    const context = await browser.newContext({ viewport: { width, height }, colorScheme: scheme, reducedMotion: 'no-preference', serviceWorkers: 'block' });
    const page = await context.newPage(); await setup(page); await gotoApp(page);
    ...
    if (representative.has(label)) {
      ...
      await page.getByTestId('gps-button').click(); await page.getByTestId('origin-edit').waitFor({ state: 'visible' });
      await snap(page, `${label}-${scheme}-gps-origin`, { viewport: label, scheme, state: 'GPS_ORIGIN' });
      await setManualOrigin(page); await page.getByTestId('trip-sheet').waitFor({ state: 'visible' });
    ...
    if (representative.has(label)) {
      for (const mode of modes) {
        ...
        if (mode === 'bus') await page.getByTestId('provider-bus').waitFor({ state: 'visible' }); else await page.getByTestId('trip-sheet').waitFor({ state: 'visible' });
        await snap(page, `${label}-${scheme}-mode-${mode}`, { viewport: label, scheme, state: `MODE_${mode.toUpperCase()}` });
      }
      await page.locator('[data-mode="bus"]').click(); await page.getByTestId('provider-bus').waitFor({ state: 'visible' });
      await snap(page, `${label}-${scheme}-bus-unavailable`, { viewport: label, scheme, state: 'UNAVAILABLE_DATA_BUS' });
      await page.locator('[data-mode="app"]').click(); await page.getByTestId('provider-uber').waitFor({ state: 'visible' });
      await page.getByTestId('provider-uber').click(); await page.getByTestId('external-confirmation').waitFor({ state: 'visible' });
      ...
      if (await voice.count()) { await voice.click(); await page.waitForTimeout(250); await snap(page, `${label}-${scheme}-voice`, { viewport: label, scheme, state: 'VOICE_IF_ENABLED' }); }
      await context.setOffline(true); await page.evaluate(() => dispatchEvent(new Event('offline'))); await page.getByTestId('offline-banner').waitFor({ state: 'visible' });
...
for (const scheme of ['light','dark']) {
  ...
  const page = await context.newPage(); await setup(page, { destinationMode: 'unverified' }); await gotoApp(page);
  await page.getByTestId('destination-input').fill('No verificado'); await page.getByTestId('destination-results').waitFor({ state: 'visible' });
  await snap(page, `390x844-${scheme}-error-unverified`, { viewport: '390x844', scheme, state: 'ERROR_UNVERIFIED_DESTINATION' }); await context.close();
}
for (const scheme of ['light','dark']) {
  ...
  for (const route of ['/privacy','/terms','/sources','/contact']) {
    const page = await context.newPage(); await page.goto(base + route, { waitUntil: 'domcontentloaded' }); await page.getByTestId('legal-view').waitFor({ state: 'visible' });
    await snap(page, `1440x900-${scheme}-legal-${route.slice(1)}`, { viewport: '1440x900', scheme, state: `LEGAL_${route.slice(1).toUpperCase()}` }); await page.close();
  }
  await context.close();
}
await browser.close();
fs.writeFileSync(path.join(out, 'before-capture-index.json'), `${JSON.stringify({ result: 'PASS', source: 'VERIFIED_PRODUCTION_ASSETS_WITH_CANONICAL_DATA_SHAPE_FIXTURES', production_url: base, viewports: viewports.map(v => v[0]), themes: ['light','dark'], applicable_modes: modes, auth_enabled_in_primary_shell: false, screenshots: records, captured_at: new Date().toISOString() }, null, 2)}\n`);
