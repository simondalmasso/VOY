...
describe('VOY Destination Resolution V2 - Review Blockers Tests', () => {
  ...
  test('Coordinate validity: reject invalid coordinates in remote candidate processing and accept legit numeric zero', () => {
    ...
    const context = vm.createContext({
      ...
      _parseValidCoordinateValue: null,
    ...
    const check = context._parseValidCoordinateValue;
    const process = context._remoteCandidate;

    // Coordinate validation unit tests
    assert.equal(check(null), null, 'null is invalid');
    assert.equal(check(undefined), null, 'undefined is invalid');
    assert.equal(check(''), null, 'empty string is invalid');
    assert.equal(check('   '), null, 'spaces string is invalid');
    assert.equal(check(NaN), null, 'NaN is invalid');
    assert.equal(check(Infinity), null, 'Infinity is invalid');
    assert.equal(check(-Infinity), null, '-Infinity is invalid');
    assert.equal(check(true), null, 'boolean true is invalid');
    assert.equal(check(false), null, 'boolean false is invalid');
    assert.equal(check([]), null, 'arrays are invalid');
    assert.equal(check([0]), null, 'arrays are invalid');
    assert.equal(check({}), null, 'objects are invalid');
    assert.equal(check('abc'), null, 'non-numeric strings are invalid');

    assert.equal(check(0), 0, 'numeric 0 is valid');
    assert.equal(check('0'), 0, 'string "0" is valid');
    assert.equal(check(-31.62), -31.62, 'legitimate negative numeric value is valid');
