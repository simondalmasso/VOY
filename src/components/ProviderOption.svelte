<script lang="ts">
  ...
  $: accessibleLabel = unavailableLabel ? `${option.name}: ${unavailableLabel}` : priceLabel ? `${option.name}: ${priceLabel}` : option.name;
</script>
...
<div class="provider-item" role="listitem">
  ...
  {:else if !option.available}
    <button type="button" class="provider provider-info disabled" class:provider-unavailable={unavailable} data-testid={`provider-${option.id}`} data-price-kind={option.price.kind} aria-disabled="true" aria-label={accessibleLabel}>
  ...
  {:else}
    <article class="provider provider-info" data-testid={`provider-${option.id}`} data-price-kind={option.price.kind} aria-label={accessibleLabel}>
      <span class="provider-main">
        <strong>{option.name}</strong>
        <small>{option.detail}</small>
      </span>
      {#if hasMeta}<span class="provider-meta">{#if priceLabel}<strong>{priceLabel}</strong>{/if}{#if option.etaMin !== null && option.etaMin > 0}<small>{option.etaMin} min</small>{/if}</span>{/if}
    </article>
  {/if}
</div>
