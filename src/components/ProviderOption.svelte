...
{#if actionable}
  <button type="button" class="provider" on:click={() => onChoose(option)} data-testid={`provider-${option.id}`} aria-label={`Abrir ${option.name} con confirmación`}>
    <span class="provider-main"><strong>{option.name}</strong><small>{option.detail}</small></span>
    <span class="provider-action">Abrir {option.name}</span>
  </button>
{:else}
  <article role="listitem" class="provider provider-info" class:disabled={!option.available} data-testid={`provider-${option.id}`} aria-label={priceLabel ? `${option.name}: ${priceLabel}` : option.name}>
    <span class="provider-main"><strong>{option.name}</strong><small>{option.detail}</small></span>
