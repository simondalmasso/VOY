<script lang="ts">
  ...
  function expandedSnap(): SheetSnap {
    return nextExpandedSnap(snap);
  }

  function collapsedSnap(): SheetSnap | null {
    return nextCollapsedSnap(snap);
  }

  function expand(): void {
    onSnapChange(expandedSnap());
  }

  function collapse(): void {
    const next = collapsedSnap();
  ...
  function recordSample(y: number, time: number): void {
    ...
    samples = samples.filter(sample => sample.time >= cutoff).slice(-6);
  }
  ...
  function springTo(targetY: number, initialVelocityPxMs: number, momentum: boolean): void {
    ...
    const step = (now: number) => {
      ...
      if (Math.abs(position - targetY) <= 0.5 && Math.abs(velocity) <= 4) {
        finishVisualSettle(targetY);
        return;
      }
  ...
  function moveDrag(event: PointerEvent): void {
    ...
    const targets = snapTargets();
    const direct = dragStartTransformY + dragDistance;
    applyTransform(rubberBand(direct, targets.expanded, targets.peek));
    recordSample(event.clientY, event.timeStamp);
  }
  ...
  function keyboardSnap(target: SheetSnap | null): void {
    ...
    requestAnimationFrame(() => {
      if (motionState === 'keyboard') motionState = 'idle';
    });
  }
  ...
  function handleKeydown(event: KeyboardEvent): void {
    ...
    if (event.key === 'Enter' || event.key === ' ') {
      ...
      keyboardSnap(snap === 'expanded' ? collapsedSnap() : expandedSnap());
    }
  }
</script>
...
<section
  ...
  class="sheet"
  class:dragging
  class:settling={motionState === 'settling'}
  aria-labelledby="decision-title"
  data-testid="trip-sheet"
  data-snap={snap}
  data-motion-state={motionState}
  data-release-mode={releaseMode}
>
  <button
    type="button"
    class="sheet-handle"
    ...
    on:pointercancel={(event) => finishDrag(event, true)}
