...
export async function providerOptions(route: RouteResult | null, selectedMode: TravelMode, coverageKey: CoverageKey = '_default'): Promise<ProviderOptionModel[]> {
  ...
  const all: ProviderOptionModel[] = [
    { id: 'uber', name: uber?.name || 'Uber', mode: 'app', available: currentApp(uber), etaMin: null, price: { kind: 'app_only' }, detail: currentApp(uber) ? 'Presencia territorial verificada; tarifa y disponibilidad puntual se consultan en la app.' : 'Presencia territorial actual no verificada.', external: true, rank: 20 },
    { id: 'didi', name: didi?.name || 'DiDi', mode: 'app', available: currentApp(didi), etaMin: null, price: { kind: 'app_only' }, detail: currentApp(didi) ? 'Presencia territorial verificada; tarifa y disponibilidad puntual se consultan en la app.' : 'Presencia territorial actual no verificada.', external: true, rank: 21 },
