...
type TrustBody = {
  ok?: boolean;
  error?: string;
  operational_bus_activation?: boolean;
  mobility_database_role?: string;
  gtfs_validator?: { version?: string; sha256?: string };
  santa_fe?: { bus_activation?: boolean; sources?: Array<{ trust_status?: string }> };
};
...
describe('mobility trust API', () => {
  test('is read-only and keeps Santa Fe bus activation off', async () => {
    ...
    const body = await response.json() as TrustBody;
