...
describe('provider ranking and price truthfulness', () => {
  test('apps remain APP_ONLY, regulated modes are informational and bus remains disabled', async () => {
    ...
    expect(apps.filter(option => option.id === 'uber' || option.id === 'didi').every(option => option.price.kind === 'app_only' && option.external)).toBe(true);
    ...
    expect(bus).toHaveLength(1); expect(bus[0]?.available).toBe(false); expect(bus[0]?.detail).toContain('no calcula ni sugiere');
  });
});
