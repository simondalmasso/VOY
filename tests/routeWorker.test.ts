...
describe('route API boundary', () => {
  ...
  test('valid upstream response is normalized and never cached publicly', async () => {
    globalThis.fetch = (async () => new Response(JSON.stringify({ code: 'Ok', routes: [{ distance: 3200, duration: 630, geometry: { coordinates: [[-60.70, -31.63], [-60.695, -31.635], [-60.69, -31.64]] } }] }), { status: 200 })) as unknown as typeof fetch;
