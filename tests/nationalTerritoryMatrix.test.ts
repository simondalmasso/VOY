...
describe('ORDER-046 national territory matrix', () => {
  for (const item of CASES) {
    test(`${item.locality} resolves to ${item.provinceIsoId} without inheriting foreign local coverage`, () => {
      ...
      if (item.coverageKey === 'santa-fe') {
        expect(territory).toMatchObject({ coverageLevel: 'LOCAL_VERIFIED', source: 'voy_local_profile' });
      } else {
        expect(territory).toMatchObject({ coverageLevel: 'NATIONAL_BASE', source: 'georef_v2' });
      }
    });
  }
  ...
  test('a Santa Fe homonym outside province 82 never receives the Santa Fe city overlay', () => {
    ...
    expect(territory?.coverageLevel).toBe('NATIONAL_BASE');
  });
