...
describe('ORDER-046 Argentina territorial core', () => {
  ...
  test('only exact Santa Fe city context may select the verified Santa Fe local overlay', () => {
    ...
    expect(city).toMatchObject({ coverageKey: 'santa-fe', coverageLevel: 'LOCAL_VERIFIED', source: 'voy_local_profile' });
    expect(rosario).toMatchObject({ coverageKey: '_default', coverageLevel: 'NATIONAL_BASE', source: 'georef_v2' });
  });
